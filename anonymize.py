...
class Anonymizer:
    ...
    def component(self, category, original, faker_method):
        key = normalized(original)
        if key not in self.cache[category]:
            attempt = 0
            while True:
                fake = self.fake(category, f"{key}|{attempt}")
                replacement = getattr(fake, faker_method)()
                if normalized(replacement) != key:
                    break
                attempt += 1
